...
struct mydsp : public dsp {
	...
	void metadata(Meta* m) { 
		...
		m->declare("demos.lib/gate_demo:author", "Julius O. Smith III");
		m->declare("demos.lib/gate_demo:licence", "MIT");
		m->declare("demos.lib/name", "Faust Demos Library");
		m->declare("demos.lib/version", "1.2.0");
		m->declare("description", "Gate demo application.");
		m->declare("filename", "noise_gate.dsp");
	...
	virtual void buildUserInterface(UI* ui_interface) {
		...
		ui_interface->declare(&fHslider0, "tooltip", "When         the signal level falls below the Threshold (expressed in dB), the signal is         muted");
		...
		ui_interface->declare(&fHslider1, "tooltip", "Time constant in MICROseconds (1/e smoothing time) for the gate     gain to go (exponentially) from 0 (muted) to 1 (unmuted)");
		ui_interface->declare(&fHslider1, "unit", "us");
		...
		ui_interface->declare(&fHslider2, "tooltip", "Time constant in ms (1/e smoothing time) for the gain to go     (exponentially) from 1 (unmuted) to 0 (muted)");
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		float fSlow1 = std::pow(1e+01f, 0.05f * static_cast<float>(fHslider0));
		float fSlow2 = std::max<float>(fConst1, 1e-06f * static_cast<float>(fHslider1));
		float fSlow3 = std::max<float>(fConst1, 0.001f * static_cast<float>(fHslider2));
		float fSlow4 = std::min<float>(fSlow2, fSlow3);
		int iSlow5 = std::fabs(fSlow4) < 1.1920929e-07f;
		float fSlow6 = ((iSlow5) ? 0.0f : std::exp(-(fConst1 / ((iSlow5) ? 1.0f : fSlow4))));
		float fSlow7 = 1.0f - fSlow6;
		int iSlow8 = static_cast<int>(fConst0 * std::max<float>(fConst1, 0.001f * static_cast<float>(fHslider3)));
		int iSlow9 = std::fabs(fSlow3) < 1.1920929e-07f;
		float fSlow10 = ((iSlow9) ? 0.0f : std::exp(-(fConst1 / ((iSlow9) ? 1.0f : fSlow3))));
		int iSlow11 = std::fabs(fSlow2) < 1.1920929e-07f;
		float fSlow12 = ((iSlow11) ? 0.0f : std::exp(-(fConst1 / ((iSlow11) ? 1.0f : fSlow2))));
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			...
			fRec1[0] = std::fabs(fTemp4 + std::fabs(fTemp1)) * fSlow7 + fRec1[1] * fSlow6;
			int iTemp5 = fRec1[0] > fSlow1;
			iVec0[0] = iTemp5;
			iRec2[0] = std::max<int>(iSlow8 * (iTemp5 < iVec0[1]), iRec2[1] + -1);
			float fTemp6 = std::fabs(std::max<float>(static_cast<float>(iTemp5), static_cast<float>(iRec2[0] > 0)));
			float fTemp7 = ((fTemp6 > fRec0[1]) ? fSlow12 : fSlow10);
			fRec0[0] = fTemp6 * (1.0f - fTemp7) + fRec0[1] * fTemp7;
			fHbargraph0 = static_cast<FAUSTFLOAT>(2e+01f * std::log10(std::max<float>(1.1754944e-38f, fRec0[0])));
			float fTemp8 = fTemp1;
			fRec4[0] = fSlow7 * std::fabs(fTemp4 + std::fabs(fTemp8)) + fSlow6 * fRec4[1];
			int iTemp9 = fRec4[0] > fSlow1;
			iVec1[0] = iTemp9;
			iRec5[0] = std::max<int>(iSlow8 * (iTemp9 < iVec1[1]), iRec5[1] + -1);
			float fTemp10 = std::fabs(std::max<float>(static_cast<float>(iTemp9), static_cast<float>(iRec5[0] > 0)));
			float fTemp11 = ((fTemp10 > fRec3[1]) ? fSlow12 : fSlow10);
			fRec3[0] = fTemp10 * (1.0f - fTemp11) + fRec3[1] * fTemp11;
			output0[i0] = static_cast<FAUSTFLOAT>(((iSlow0) ? fTemp0 : fRec3[0] * fTemp8));
			output1[i0] = static_cast<FAUSTFLOAT>(((iSlow0) ? fTemp2 : fRec3[0] * fTemp3));
			fRec1[1] = fRec1[0];
			iVec0[1] = iVec0[0];
			iRec2[1] = iRec2[0];
			fRec0[1] = fRec0[0];
	...
	#define FAUST_FILE_NAME "noise_gate.dsp"
	...
	FAUST_ADDCHECKBOX("GATE  [tooltip: Reference:         http://en.wikipedia.org/wiki/Noise_gate]/[0]/Bypass", fCheckbox0);
	FAUST_ADDHORIZONTALBARGRAPH("GATE  [tooltip: Reference:         http://en.wikipedia.org/wiki/Noise_gate]/[0]/Gate Gain", fHbargraph0, -5e+01f, 1e+01f);
	FAUST_ADDHORIZONTALSLIDER("GATE  [tooltip: Reference:         http://en.wikipedia.org/wiki/Noise_gate]/[1]/Threshold", fHslider0, -3e+01f, -1.2e+02f, 0.0f, 0.1f);
	FAUST_ADDHORIZONTALSLIDER("GATE  [tooltip: Reference:         http://en.wikipedia.org/wiki/Noise_gate]/[1]/Attack", fHslider1, 1e+01f, 1e+01f, 1e+04f, 1.0f);
	FAUST_ADDHORIZONTALSLIDER("GATE  [tooltip: Reference:         http://en.wikipedia.org/wiki/Noise_gate]/[1]/Hold", fHslider3, 2e+02f, 1.0f, 1e+03f, 1.0f);
	FAUST_ADDHORIZONTALSLIDER("GATE  [tooltip: Reference:         http://en.wikipedia.org/wiki/Noise_gate]/[1]/Release", fHslider2, 1e+02f, 1.0f, 1e+03f, 1.0f);

	#define FAUST_LIST_ACTIVES(p) \
		p(CHECKBOX, Bypass, "GATE  [tooltip: Reference:         http://en.wikipedia.org/wiki/Noise_gate]/[0]/Bypass", fCheckbox0, 0.0f, 0.0f, 1.0f, 1.0f) \
		p(HORIZONTALSLIDER, Threshold, "GATE  [tooltip: Reference:         http://en.wikipedia.org/wiki/Noise_gate]/[1]/Threshold", fHslider0, -3e+01f, -1.2e+02f, 0.0f, 0.1f) \
		p(HORIZONTALSLIDER, Attack, "GATE  [tooltip: Reference:         http://en.wikipedia.org/wiki/Noise_gate]/[1]/Attack", fHslider1, 1e+01f, 1e+01f, 1e+04f, 1.0f) \
		p(HORIZONTALSLIDER, Hold, "GATE  [tooltip: Reference:         http://en.wikipedia.org/wiki/Noise_gate]/[1]/Hold", fHslider3, 2e+02f, 1.0f, 1e+03f, 1.0f) \
		p(HORIZONTALSLIDER, Release, "GATE  [tooltip: Reference:         http://en.wikipedia.org/wiki/Noise_gate]/[1]/Release", fHslider2, 1e+02f, 1.0f, 1e+03f, 1.0f) \

	#define FAUST_LIST_PASSIVES(p) \
		p(HORIZONTALBARGRAPH, Gate_Gain, "GATE  [tooltip: Reference:         http://en.wikipedia.org/wiki/Noise_gate]/[0]/Gate Gain", fHbargraph0, 0.0, -5e+01f, 1e+01f, 0.0) \
...
noise_gate::noise_gate() : AudioStream(FAUST_INPUTS, new audio_block_t*[FAUST_INPUTS])
{
#ifdef NVOICES
    int nvoices = NVOICES;
...
void noise_gate::updateImp(void)
{
#if MIDICTRL
...
void noise_gate::update(void) { updateImp<FAUST_INPUTS, FAUST_OUTPUTS>(); }

void noise_gate::setParamValue(const std::string& path, float value)
{
    fUI->setParamValue(path, value);
}

float noise_gate::getParamValue(const std::string& path)
{
