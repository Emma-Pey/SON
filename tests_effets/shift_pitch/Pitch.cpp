...
license: "BSD"
name: "Pitch", "pitchShifter", "Misc Effects Library"
version: "1.0", "2.5.2"
...
struct mydsp : public dsp {
	...
	virtual void buildUserInterface(UI* ui_interface) {
		...
		ui_interface->addHorizontalSlider("xfade (samples)", &fHslider1, FAUSTFLOAT(1e+01f), FAUSTFLOAT(1.0f), FAUSTFLOAT(1e+04f), FAUSTFLOAT(1.0f));
		ui_interface->closeBox();
	}
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			...
			float fTemp0 = std::min<float>(fSlow1 * fRec0[0], 1.0f);
	...
	#define FAUST_FILE_NAME "Pitch.dsp"
	...
	FAUST_ADDHORIZONTALSLIDER("Pitch Shifter/xfade (samples)", fHslider1, 1e+01f, 1.0f, 1e+04f, 1.0f);

	#define FAUST_LIST_ACTIVES(p) \
		p(HORIZONTALSLIDER, shift_(semitones), "Pitch Shifter/shift (semitones)", fHslider0, 0.0f, -12.0f, 12.0f, 0.1f) \
		p(HORIZONTALSLIDER, xfade_(samples), "Pitch Shifter/xfade (samples)", fHslider1, 1e+01f, 1.0f, 1e+04f, 1.0f) \
...
Pitch::Pitch() : AudioStream(FAUST_INPUTS, new audio_block_t*[FAUST_INPUTS])
{
#ifdef NVOICES
    int nvoices = NVOICES;
...
void Pitch::update(void) { updateImp<FAUST_INPUTS, FAUST_OUTPUTS>(); }

void Pitch::setParamValue(const std::string& path, float value)
{
    fUI->setParamValue(path, value);
}

float Pitch::getParamValue(const std::string& path)
{
